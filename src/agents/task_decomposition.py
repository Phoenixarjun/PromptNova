...
class TaskDecomposition(PromptAgent):
    ...
    def refine(self, user_input: str, sub_steps: List[str] = None, **kwargs) -> str:
        ...
        task_decomposition_template = PromptTemplate(
            ...
            template="""You are an expert prompt engineer with 25+ years of experience. Transform the following raw, improper user input into a top-tier, expert-level prompt optimized for Gemini AI. The refined prompt should be clear, concise, specific, actionable, and structured with precise instructions. Use task decomposition prompting: instruct the AI to break the task into sub-steps ({sub_steps_str}), solving each to build a complete response.
